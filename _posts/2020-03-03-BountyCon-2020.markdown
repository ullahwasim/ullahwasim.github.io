...
And this works on AWS with Ubuntu. Same method wasn't work on local kali Linux machine because you have to zip the plaintext file to crack password and zip program of ubuntu and kali wasn't same(there is same difference). Protected files was zipped on Ubuntu.

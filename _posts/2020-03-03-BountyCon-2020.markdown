---
layout: post
title:  "Facebook BountyCon 2020 Writeup"
...
Before coming to the right solutions to the challenges. I tried a lot of different ways to solve the challenges. Below are the solutions that I used to solve the challenges.
# Secure Login
```
...
Starting from the 1st Challange which had a description stated above.
...
Build the apk file again using `apktool b "shake_it" -o "shake_it1.apk"`.If you install a newly created apk file you will get a certificate error. So we have to sign the apk file. For this, First, create a key using keytool `keytool -v -genkey -keystore shake_it1.keystore -alias shake_it1 -keyalg RSA -keysize 1024 -sigalg SHA1withRSA -validity 356`. Enter anything in the fields
...
```
line `(*(code *)puVar1)(); ` looks strange. Here it is calling some method
, so lets check what it is calling.
...
set breakpoint after line "call *%rdx".In my case it is.
...
I patch the file by changing program to jmp 0x0040101a
```
r2 -w ./ghidra_is_cool
s 0x401000
wa jmp 0x0040101a
...
So, I analyzed the apk with gdb ,apktool, and Jadx.

![](/images/Screenshot_6.png){:class="img-responsive"}


Pressing any emoji text will appear as something like ":abc-abc-abc:". Flag format for this challenge is bountycon-... as given in the details of the task. so the flag should be like ":bountycon-abc-abc:" and text will convert to some sort of emoji.

After checking the JAVA code of apk file. There was nonthing interesting in the apk file. Everything was done in `libparse.so` file. So, I started to debug it.
...
c 10 # length of "bountycon-" is 10 if message prints on the app then a is wrong guess.
# Enter :bountycon-3:
c 10 # gdb will be on breakpoint under the if condition as input is correct so 3 is the right input towards the flag.
c
# Enter :bountycon-3a: as flag for this challenge starts with bountycon-...
c 11 # length of "bountycon-3" is 11 if message prints on the app then a is wrong guess.
# Enter :bountycon-3m:
...
After login into the website. On the website, there was a page that an admin could access only.
Saying 
```
FLAGship product
...
if hash is same then email is put on SQL query and SQL query is executed to get the user and return it. 


There are two things to note.


1- Secret key concatenate with user email then MD5 is done.

2- SQL query is vulnerable to SQL injection.
...
So, the idea was to make a valid hash of message with SQL injection(secret_cookie_key+new_message where new_message is containing `" or 'role'="admin"-- -` in the last) to log in as admin by doing SQLi injection.
...
there was the three snapshot of this page. From which one is `https://web.archive.org/web/20191231221341/https://www.glue.bar/mattsstringreverser/`
...
The next day, I tried the same way but making things similar to `glue.bar` server e.g Ubuntu server on AWS instead of a local kali Linux server.

and this works on AWS with ubuntu.
...
Open HTML file in chrome.Open developer tools and in the Source tab.open js file. Click on the pretty print button to Beautify the code.

![](/images/Untitled3.png){:class="img-responsive"}
...
2- Bad request: message padding error 

3- SyntaxError: Missing parentheses in call to 'print'. Did you mean print(ptext)?
...
It has no information about the API key. Its showing telling us how to use their API.
...
Clicking on `login` leads to another website.
...
and Clicking on cancel leads us to `https://ik6939fg0f.execute-api.eu-west-2.amazonaws.com/default/qx9nc2?AUTH_TOKEN=%3BFZbocBdRNUdHNe2sYnCX05OsnSbRqtPRscvllzbLTB8%3D`

URL decode the following `%3BFZbocBdRNUdHNe2sYnCX05OsnSbRqtPRscvllzbLTB8%3D` leads to `;FZbocBdRNUdHNe2sYnCX05OsnSbRqtPRscvllzbLTB8=` before is `;` is username which is empty and then hmac-sha256 is done on username with some password which we dont know.but what if password is weak.lets bruteforce it.

I hashcat with `rockyou.txt` file. And found the password which was `ilovefacebook`

![](/images/Screenshot_29.png){:class="img-responsive"}

I made the hmac-sha256 for `admin` using the password `ilovefacebook` and entered that hash to AUTH_TOKEN e.g `https://ik6939fg0f.execute-api.eu-west-2.amazonaws.com/default/qx9nc2?AUTH_TOKEN=mEE3YZsG%2FlrlYlmdMlK8tqmA8CKWRwTr9%2FNDcipJF4c%3D` which gave us the flag.
...
First, File contains the `flag` variable which we want to get it. Second, it using `pickle` which is not secure from object injection.
